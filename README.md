...
> crítico. El catálogo de recetas e ingredientes en `recetas.js` / `ingredientes.js` es de ejemplo —
...
valor fijo (ver `procesarVentas` en `depletion.js`).

Cada noche, el scheduler corre `correrDepletion()`: trae las ventas del día por la API de Loyverse,
...
scheduler.js          arranca el cron nocturno (y un heartbeat cada hora)
  └─ depletion.js      orquesta correrDepletion(): ventas → consumo → stock → alerta
       ├─ recetas.js       receta = ingrediente → cantidad (ejemplo)
       ├─ mapeo.js         nombre del item en el POS → clave de receta (ejemplo)
       └─ ingredientes.js  ingrediente → uuid de variante en el POS, factor, mínimo (ejemplo)
```

`leche` y `te_verde_negro` son claves especiales dentro de una receta: en vez de apuntar a un
ingrediente fijo, `depletion.js` las resuelve contra el modificador que trae el pedido
(`MILK_VARIANTS` / `TEA_VARIANTS` en `ingredientes.js`).
